...
public class Personenverwaltung {
    ...
    public void getUser(String[] splittedName){
        boolean foundUser = false;
        for (Person p : personenList) {

            if (splittedName[0].equals(p.name)){
                if (splittedName[1].equals(p.surname)){
                    foundUser = true;
        ...
        if (!foundUser){
            throw new NullPointerException("Diese Person existiert nicht");
        }
    }
}
